...
cd /root/sandplanet
# STACK=marine backs up the Sandplanet Marine instance instead
# (MARINE_BUILD_BRIEF.md §3): its own service, env file and file names.
STACK="${STACK:-planet}"
if [ "$STACK" = "marine" ]; then
  COMPOSE="docker compose -f docker-compose.prod.yml -f docker-compose.marine.yml"
  DBSVC=db-marine; ENVFILE=./.env.marine
else
  COMPOSE="docker compose -f docker-compose.prod.yml"
  DBSVC=db; ENVFILE=./.env
fi
DIR=/root/backups
KEEP_LOCAL_DAYS=14
KEEP_REMOTE_DAYS=35
STAMP=$(date +%Y%m%d-%H%M)
NAME="${STACK}-${STAMP}.sql.gz"
mkdir -p "$DIR"
...
# the script outright).
env_val() { sed -n "s/^$1=//p" "$ENVFILE" | head -1 | tr -d '"'"'"'"' ; }
...
restore_test() {
  ...
  $COMPOSE exec -T "$DBSVC" psql -U "$PGUSER_" -d postgres \
      -c "DROP DATABASE IF EXISTS $scratch;" >/dev/null
  $COMPOSE exec -T "$DBSVC" psql -U "$PGUSER_" -d postgres \
      -c "CREATE DATABASE $scratch;" >/dev/null
  if ! gunzip -c "$file" | $COMPOSE exec -T "$DBSVC" psql -q -v ON_ERROR_STOP=1 \
  ...
  local docs emps
  docs=$($COMPOSE exec -T "$DBSVC" psql -tAc \
      "SELECT count(*) FROM core_document;" -U "$PGUSER_" -d "$scratch")
  emps=$($COMPOSE exec -T "$DBSVC" psql -tAc \
...
if [ "${1:-}" = "--test-only" ]; then
  newest=$(ls -t "$DIR"/"$STACK"-*.sql.gz 2>/dev/null | head -1 || true)
...
log "dumping $PGDB_"
$COMPOSE exec -T "$DBSVC" pg_dump -U "$PGUSER_" "$PGDB_" | gzip -9 > "$DIR/$NAME"
...
find "$DIR" -name "$STACK-*.sql.gz" -mtime +$KEEP_LOCAL_DAYS -delete
log "local copies: $(ls -1 "$DIR"/"$STACK"-*.sql.gz 2>/dev/null | wc -l)"

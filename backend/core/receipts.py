...
def next_receipt_no():
    ...
    from .models import RentalReceipt, TradingReceipt
    n = (OfficialReceipt.objects.count() + TradingReceipt.objects.count()
         + RentalReceipt.objects.count() + 1)
    while (OfficialReceipt.objects.filter(receipt_no=f"OR-{n:04d}").exists()
           ...
           or RentalReceipt.objects.filter(receipt_no=f"OR-{n:04d}").exists()):
        n += 1
    return f"OR-{n:04d}"

...
def document_revise(request, ref):
    ...
    # Lock the MR while a PR is being raised against it — amending recreates its
    # lines and would orphan the PR (owner 2026-07-31). Purchasing must finalise
    # or release the PR first, or the new items go on a separate MR.
    if doc.doc_type == "MR":
        from .procurement import mr_live_pr_refs
        pr_refs = mr_live_pr_refs(doc)
        if pr_refs:
            return Response({"detail":
                f"{', '.join(pr_refs)} is being raised against this MR by "
                f"Purchasing — it can't be amended until that's finalised or "
                f"released. For extra items, raise a new MR."}, status=409)

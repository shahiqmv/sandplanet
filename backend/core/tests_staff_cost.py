...
class UsdPaidStaffCostTests(StaffCostBase):
    """basic_pay is held in the employee's own currency; the staff-cost ledger
    is MVR. A USD-paid man's 800 was going into an MVR site total as 800
    rufiyaa — about a fifteenth of what he costs (owner 2026-09-02,
    EMP-0060 at MXR)."""

    def setUp(self):
        super().setUp()
        CompanyParameter.objects.update_or_create(
            key="usd_mvr_rate", defaults={"value": "15.42"})
        self.usd = Employee.objects.create(
            emp_no="EMP-0060", full_name="Rifdeen Junaideen",
            basic_pay=800, currency="USD", job_category=self.cat)
        EmployeeSiteAllocation.objects.create(
            employee=self.usd, site=self.site, from_date=date(2025, 1, 1))

    def _attend_usd(self, y, m, days):
        for d in days:
            Attendance.objects.create(employee=self.usd, site=self.site,
                                      day=date(y, m, d), remark="PRESENT")

    def _row(self, y, m):
        rows, total = staff_cost.month_staff_cost(self.site, y, m)
        return next(r for r in rows if r["emp_no"] == "EMP-0060"), total

    def test_his_basic_is_converted_into_the_mvr_ledger(self):
        self._attend_usd(2026, 7, list(range(1, 27)))
        row, _ = self._row(2026, 7)
        # 800 USD × 15.42 — not 800
        self.assertEqual(row["basic_portion"], Decimal("12336.00"))

    def test_the_row_says_what_he_is_paid_in(self):
        """An MVR figure beside a USD man must not read as his salary."""
        self._attend_usd(2026, 7, [1, 2, 3])
        row, _ = self._row(2026, 7)
        self.assertEqual(row["pay_currency"], "USD")

    def test_an_mvr_worker_is_untouched(self):
        self.attend(2026, 7, list(range(1, 27)))
        rows, _ = staff_cost.month_staff_cost(self.site, 2026, 7)
        row = next(r for r in rows if r["emp_no"] == "EMP-0001")
        self.assertEqual(row["basic_portion"], Decimal("9600.00"))
        self.assertEqual(row["pay_currency"], "MVR")

    def test_the_site_total_carries_the_converted_figure(self):
        self._attend_usd(2026, 7, list(range(1, 27)))
        self.attend(2026, 7, list(range(1, 27)))
        _, total = self._row(2026, 7)
        self.assertEqual(total, Decimal("21936.00"))    # 9600 + 12336

    def test_the_conversion_follows_the_company_rate(self):
        CompanyParameter.objects.update_or_create(
            key="usd_mvr_rate", defaults={"value": "17.00"})
        self._attend_usd(2026, 7, list(range(1, 27)))
        row, _ = self._row(2026, 7)
        self.assertEqual(row["basic_portion"], Decimal("13600.00"))
